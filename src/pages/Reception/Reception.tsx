...
const Reception: React.FC = () => {
  ...
  const handleChangeAccess = (index: number) => {
    const newFloorAccess = [...floorAccess];
    newFloorAccess[index] = !newFloorAccess[index];
    dispatch(changeAccess(newFloorAccess));
  };
      ...
      <section className="access-levels">
        ...
        {floorAccess.map((floor: boolean, index: number) => (
          <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === "Enter" && handleChangeAccess(index)}
          >
      ...
      <section className="role-selection">
        ...
        {optionalroles.map((role: string, index: number) => (
          <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === "Enter" && handleSetRole(index)}
          >
            <h5>{role}</h5>
          </div>
        ))}
